...
module message_board_addr::test_end_to_end {
    ...
    #[test(initia_std = @initia_std, deployer = @message_board_addr, sender = @0x100)]
    fun test_end_to_end(
        initia_std: &signer, deployer: &signer, sender: &signer
    ) {
        let sender_addr = signer::address_of(sender);

        message_board::init_module_for_test(initia_std, deployer);
    ...
    #[
        test(
            initia_std = @initia_std,
            deployer = @message_board_addr,
            sender1 = @0x100,
            sender2 = @0x101
        )
    ]
